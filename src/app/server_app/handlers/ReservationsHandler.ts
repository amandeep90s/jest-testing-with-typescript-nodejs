...
export class ReservationsHandler {
  ...
  private async handlePost() {
    ...
    const reservationId = await this.reservationsDataAccess.createReservation(
      requestBody
    );
  ...
  private async handleGet() {
    ...
    if (id === 'all') {
      const allReservations =
        await this.reservationsDataAccess.getAllReservations();
    ...
    if (id) {
      ...
      if (reservation) {
        ...
        this.response.write(
          JSON.stringify(`Reservation with id ${id} not found`)
        );
      }
    } else {
      this.response.statusCode = HTTP_CODES.BAD_REQUEST;
  ...
  private async handlePut() {
    ...
    if (id) {
      const reservation = await this.reservationsDataAccess.getReservation(id);
      if (reservation) {
        const requestBody: Partial<Reservation> = await getRequestBody(
          this.request
        );
        if (this.isValidPartialReservation(requestBody)) {
          ...
          this.response.write(
            JSON.stringify(
              `Updated ${Object.keys(requestBody)} of reservation ${id}`
            )
          );
        } else {
          this.response.statusCode = HTTP_CODES.BAD_REQUEST;
